 ...
 * Min-Min Energy strategy (similar to classic Min-Min), it assigns the short 
 * task (minimum) to the most energy-efficienty resource. 
 * 
 * Author: Fabio Coutinho
 * Date: September 2014
 */
public class MinMinEnergyCaseStudies extends GridSim {
	...
	public MinMinEnergyCaseStudies(String name, int numberOfResources, int typeOfScenario, int scenarioID, String tasksPath) throws Exception {
	...
	/**
	 *  Returns the minimum task
	 */
	public Gridlet getMinTask() {
		double minLength = 999999999;
		Gridlet minGridlet = null;
		for (int i = 0; i < this.taskList.size(); i++)
		{
			Gridlet gridlet = (Gridlet) this.taskList.get(i);
			if ( gridlet.getGridletLength() < minLength )
			{
				minLength = gridlet.getGridletLength();
				minGridlet = gridlet;
			}
		}
		return minGridlet;
	...
	/**
	 *  Returns the resource id list sorted by eee value (based on SPEC power)
	 */
	...
	public void allocTasksUpToFillResource(Integer resourceId)
	{
		...
		while ( (taskList.size() > 0) && (load < 100) ) 
		{
			...
			if (resChar != null)
			{
				...
				// assigns the minimum task to resource 
    			Gridlet task = getMinTask();
	...
	//Initializes the algorithm MinMinEnergy
	public static void main(String[] args) {
		try 
		{
	        ...
	        System.out.println("Initializing MinMinEnergy GridSim Simulation, scenario " + scenarioID_);
	        ...
	        MinMinEnergyCaseStudies cs1 = new MinMinEnergyCaseStudies("MinMinEnergyCaseStudies", 5, typeOfScenario_, scenarioID_, tasksPath_);
